...
This document records the construction of `aws_ids_testbed_08` in chronological order. Historical work performed in versions 03, 04, and 05 is represented here using the final version 06 commands wherever that functionality is still present.
...
1. Create the final project directory:
   ...
   ```text
   AWS_IDS_TestBed/aws_ids_testbed_08
   ```

2. Create and activate the Python environment:

   ```bash
   cd /usagers3/sanazb/Projects/AWS_IDS_TestBed/aws_ids_testbed_08
...
5. Create the Python package and configuration components:
   ...
   - `aws_ids_testbed_08/__init__.py`
   - `config.yaml`: AWS, SSH, AMI, subnet, security-group, and instance settings
   - `aws_ids_testbed_08/config.py`: YAML configuration loader
   - `aws_ids_testbed_08/inventory.py`: EC2 inventory management
   - `inventory.yaml`: instance IDs, states, addresses, and DNS names
   - `aws_ids_testbed_08/ec2_lab.py`: EC2 creation, refresh, and termination
   - `aws_ids_testbed_08/remote_settings.py`: SSH key and address selection
   - `aws_ids_testbed_08/remote_runner.py`: Paramiko and SCP execution
   - `aws_ids_testbed_08/setup_service.py`: remote setup-script execution
   - `aws_ids_testbed_08/cli.py`: main command-line controller

6. Add the basic inspection commands:

   ```bash
   python -m aws_ids_testbed_08.cli status
   python -m aws_ids_testbed_08.cli show-config
   python -m aws_ids_testbed_08.cli show-inventory
   ```
...
5. Create, refresh, and set up the victim in that order:
   ...
   ```bash
   python -m aws_ids_testbed_08.cli create-victim
   python -m aws_ids_testbed_08.cli refresh-victim
   python -m aws_ids_testbed_08.cli setup-victim
   ```
...
5. Create, refresh, and set up the IDS instance:
   ...
   ```bash
   python -m aws_ids_testbed_08.cli create-ids
   python -m aws_ids_testbed_08.cli refresh-ids
   python -m aws_ids_testbed_08.cli setup-ids
   ```
...
3. Create, refresh, and set up the attacker:
   ...
   ```bash
   python -m aws_ids_testbed_08.cli create-attacker
   python -m aws_ids_testbed_08.cli refresh-attacker
   python -m aws_ids_testbed_08.cli setup-attacker
   ```
...
1. Create `aws_ids_testbed_08/capture_service.py` to automate the first benign capture.
2. Add capture and verification commands:

   ```bash
   python -m aws_ids_testbed_08.cli capture-benign-http
   python -m aws_ids_testbed_08.cli verify-benign-pcap
   ```

3. Create `aws_ids_testbed_08/traffic_service.py`.
4. Add the simple benign traffic command:

   ```bash
   python -m aws_ids_testbed_08.cli generate-benign-http
   ```

5. Add general traffic generation:

   ```bash
   python -m aws_ids_testbed_08.cli generate-traffic 1 \
     --requests 100 \
     --concurrency 5

   python -m aws_ids_testbed_08.cli generate-traffic 2 \
     --requests 1000 \
     --concurrency 50

   python -m aws_ids_testbed_08.cli generate-traffic 3 \
     --packet-count 1000 \
     --port 80
   ```
...
1. Create `aws_ids_testbed_08/ids_receiver_app.py`, a FastAPI application with health-check and PCAP-upload endpoints.
2. Create `scripts/start_ids_receiver.sh` to run Uvicorn on `0.0.0.0:8000`.
3. Create `aws_ids_testbed_08/ids_receiver_service.py` to deploy, start, check, and inspect the receiver.
4. Deploy and start the receiver after completing `setup-ids`:

   ```bash
   python -m aws_ids_testbed_08.cli deploy-ids-receiver
   python -m aws_ids_testbed_08.cli ids-start-receiver
   ```
...
6. List PCAP files received by IDS:
   ...
   ```bash
   python -m aws_ids_testbed_08.cli ids-list-received-pcaps
   ```

## Phase 9 — Configure the Attacker

1. Create `aws_ids_testbed_08/attacker_config_service.py` to generate `/opt/aws_ids_testbed/config/attacker.env` on the attacker.
...
3. Configure and verify the attacker target:
   ...
   ```bash
   python -m aws_ids_testbed_08.cli configure-attacker-victim-url
   python -m aws_ids_testbed_08.cli attacker-verify-victim-url
   ```

4. Create `scripts/generate_traffic.sh` for attacker-side traffic generation.
5. Create `aws_ids_testbed_08/attacker_traffic_service.py` to deploy and run the script.
6. Deploy and verify it:

   ```bash
   python -m aws_ids_testbed_08.cli deploy-attacker-traffic
   python -m aws_ids_testbed_08.cli verify-attacker-traffic
   ```

7. Run the three configured traffic types:

   ```bash
   python -m aws_ids_testbed_08.cli attacker-generate-traffic 1 \
     --requests 100 \
     --concurrency 5

   python -m aws_ids_testbed_08.cli attacker-generate-traffic 2 \
     --requests 1000 \
     --concurrency 50

   python -m aws_ids_testbed_08.cli attacker-generate-traffic 3 \
     --packet-count 100 \
     --port 80
   ```
...
1. Create `aws_ids_testbed_08/victim_config_service.py`.
...
3. Configure and verify the victim after the IDS inventory is available:
   ...
   ```bash
   python -m aws_ids_testbed_08.cli configure-victim-ids-url
   python -m aws_ids_testbed_08.cli victim-verify-ids-url
   ```

4. Add scenario-label control:

   ```bash
   python -m aws_ids_testbed_08.cli victim-set-scenario benign_http
   python -m aws_ids_testbed_08.cli victim-set-scenario dos_http_flood
   python -m aws_ids_testbed_08.cli victim-set-scenario dos_syn_flood
   ```

## Phase 11 — General Victim PCAP Sender

1. Create `scripts/send_pcap_to_ids.sh` to upload a PCAP and its scenario label to IDS.
2. Create `aws_ids_testbed_08/victim_sender_service.py`.
3. Deploy and verify the sender:

   ```bash
   python -m aws_ids_testbed_08.cli deploy-victim-sender
   python -m aws_ids_testbed_08.cli verify-victim-sender
   ```

4. Send a specific existing PCAP:

   ```bash
   python -m aws_ids_testbed_08.cli victim-send-pcap \
...
5. Send every completed PCAP in the pending folder:
   ...
   ```bash
   python -m aws_ids_testbed_08.cli victim-send-pending-pcaps
   ```
...
2. Create `aws_ids_testbed_08/victim_capture_service.py`.
3. Deploy and verify the script:

   ```bash
   python -m aws_ids_testbed_08.cli deploy-victim-capture
   python -m aws_ids_testbed_08.cli verify-victim-capture
   ```
...
4. Capture benign HTTP traffic:
   ...
   ```bash
   # Terminal 1
   python -m aws_ids_testbed_08.cli victim-capture-scenario \
     --scenario benign_http \
     --seconds 20
...
7. List PCAPs in `writing`, `pending`, `sent`, and `failed`:
   ...
   ```bash
   python -m aws_ids_testbed_08.cli victim-list-pcaps
   ```
...
3. Create `aws_ids_testbed_08/victim_capture_agent_service.py`.
4. Add the complete capture-agent lifecycle:

   ```bash
   python -m aws_ids_testbed_08.cli deploy-victim-capture-agent
   python -m aws_ids_testbed_08.cli verify-victim-capture-agent
   python -m aws_ids_testbed_08.cli victim-start-capture-agent
   python -m aws_ids_testbed_08.cli victim-capture-agent-status
   python -m aws_ids_testbed_08.cli victim-stop-capture-agent
   ```
...
3. Create `aws_ids_testbed_08/victim_sender_agent_service.py`.
...
5. Add the complete sender-agent lifecycle:
   ...
   ```bash
   python -m aws_ids_testbed_08.cli deploy-victim-sender-agent
   python -m aws_ids_testbed_08.cli verify-victim-sender-agent
   python -m aws_ids_testbed_08.cli victim-start-sender-agent
   python -m aws_ids_testbed_08.cli victim-sender-agent-status
   python -m aws_ids_testbed_08.cli victim-stop-sender-agent
   ```
...
1. Add one command to start both agents:
   ...
   ```bash
   python -m aws_ids_testbed_08.cli victim-start-agents
   ```
...
2. Add individual and combined status commands:
   ...
   python -m aws_ids_testbed_08.cli victim-capture-agent-status
   python -m aws_ids_testbed_08.cli victim-sender-agent-status
   python -m aws_ids_testbed_08.cli victim-agents-status
   ```

3. Add one command that stops the sender and then the capture agent:

   ```bash
   python -m aws_ids_testbed_08.cli victim-stop-agents
   ```

## Phase 16 — One-Command Scenario Orchestration

1. Add the `run-scenario` workflow to `aws_ids_testbed_08/cli.py`.
...
3. Run benign HTTP:
   ...
   ```bash
   python -m aws_ids_testbed_08.cli run-scenario 1 \
     --requests 100 \
     --concurrency 5 \
...
4. Run controlled HTTP-flood traffic:
   ...
   ```bash
   python -m aws_ids_testbed_08.cli run-scenario 2 \
     --requests 1000 \
     --concurrency 50 \
     --auto-stop-seconds 15
   ```
...
5. Run controlled SYN traffic:
   ...
   ```bash
   python -m aws_ids_testbed_08.cli run-scenario 3 \
     --packet-count 100 \
     --port 80 \
     --auto-stop-seconds 15
   ```
...
1. Terminate all three roles when a test cycle is finished:
   ...
   ```bash
   python -m aws_ids_testbed_08.cli terminate-victim
   python -m aws_ids_testbed_08.cli terminate-attacker
   python -m aws_ids_testbed_08.cli terminate-ids
   ```
...
PCAP-to-CSV conversion, trained-model deployment, inference, classification, and alert presentation remain later project stages and are not implemented by the current `aws_ids_testbed_08` code.
