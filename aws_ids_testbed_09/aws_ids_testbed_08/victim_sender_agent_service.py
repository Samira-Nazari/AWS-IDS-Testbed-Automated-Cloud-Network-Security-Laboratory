...
def verify_victim_sender_delivery(
    project_root: Path,
    scenario: str | None = None,
) -> int:
    """Verify that pending PCAP files were delivered to the sent directory."""
    username = get_ssh_username(project_root)
    private_key_path = get_private_key_path(project_root)
    victim_public_host = get_public_host(project_root, "victim")

    sent_pattern = "*.pcap" if scenario is None else f"{scenario}_*.pcap"
    safe_sent_pattern = shlex.quote(sent_pattern)

    command = (
        "PENDING_DIR=/opt/aws_ids_testbed/pcap/pending; "
        "SENT_DIR=/opt/aws_ids_testbed/pcap/sent; "
        "pending_count=$(find \"$PENDING_DIR\" -maxdepth 1 "
        "-type f -name '*.pcap' | wc -l); "
        "sent_count=$(find \"$SENT_DIR\" -maxdepth 1 "
        "-type f -name " + safe_sent_pattern + " | wc -l); "
        "echo \"[sender] Pending PCAPs: $pending_count\"; "
        "echo \"[sender] Sent matching PCAPs: $sent_count\"; "
        "if [ \"$pending_count\" -ne 0 ]; then "
        "echo '[sender] Delivery incomplete: pending files remain.'; "
        "exit 1; "
        "fi; "
        "if [ \"$sent_count\" -eq 0 ]; then "
        "echo '[sender] No matching sent PCAP files found.'; "
        "exit 1; "
        "fi; "
        "echo '[sender] PCAP delivery verified.'"
    )
