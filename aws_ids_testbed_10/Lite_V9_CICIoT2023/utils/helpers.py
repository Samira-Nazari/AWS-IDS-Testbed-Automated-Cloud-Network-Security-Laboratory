...
def split_time_series_strict(X, y,
    ...
    return X_train, X_val, X_test, y_train, y_val, y_test
# ========================================================
def split_per_class_time_series(X, y, train_ratio=None, val_ratio=None,
                                test_ratio=None):
    """
    Per-class chronological split:
    - oldest samples -> train
    - next samples -> validation
    - newest samples -> test
    """
    if val_ratio is None or test_ratio is None:
        raise ValueError("val_ratio and test_ratio must be provided from config.py")
    ...
    X_test = X[test_idx] 
    y_test = y[test_idx] 
    logger.info("✓ Restored chronological order within train/val/test") 
    return X_train, X_val, X_test, y_train, y_val, y_test


def split_per_class_time_series_df(df, label_column='Label', timestamp_column='Timestamp',
                                   train_ratio=None, val_ratio=None,
                                   test_ratio=None):
    """
    Per-class chronological split for a DataFrame.

    This preserves the V9 attack-type split idea, but lets the split happen
    before StandardScaler and RandomForest feature selection are fitted.
    """
    if val_ratio is None or test_ratio is None:
        raise ValueError("val_ratio and test_ratio must be provided from config.py")
    if train_ratio is None:
    ...
    if timestamp_column in df.columns:
        df_ordered = df.sort_values(timestamp_column).reset_index(drop=True)
    else:
        logger.warning("Timestamp column not found; using existing row order")
        df_ordered = df.reset_index(drop=True)

    classes = df_ordered[label_column].dropna().unique()
    ...
    logger.info("✓ DataFrame per-class chronological split completed")
...
if __name__ == "__main__":
    ...
    from config.config import TEST_SIZE, VALID_SIZE, RANDOM_STATE
