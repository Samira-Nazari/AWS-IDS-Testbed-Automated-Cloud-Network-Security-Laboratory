...
from aws_ids_testbed_10.ids_inference_agent_service import (
    mark_ids_scenario_input_finished,
    prepare_ids_inference_run,
    start_ids_inference_agents,
    verify_ids_master_predictions,
    wait_for_ids_scenario_complete,
)
...
def prepare_and_start_ids_inference() -> int:
    """Start the complete IDS receiver and inference pipeline."""

    print("Starting and checking the IDS receiver...")
    receiver_status = ids_start_receiver(PROJECT_ROOT)

    if receiver_status != 0:
        return receiver_status

    print("Preparing the shared IDS inference run...")
    prepare_status = prepare_ids_inference_run(PROJECT_ROOT)

    if prepare_status != 0:
        return prepare_status

    print("Starting the IDS input and inference agents...")
    agents_status = start_ids_inference_agents(PROJECT_ROOT)

    if agents_status != 0:
        return agents_status

    print("Starting the IDS PCAP-to-CSV converter agent...")
...
def run_benign_scenario_command(args: argparse.Namespace) -> int:
    ...
    ids_status = prepare_and_start_ids_inference()
    ...
    try:
        ...
        print("Step 10: Tell IDS that scenario input is finished.")
        input_finished_status = mark_ids_scenario_input_finished(
            PROJECT_ROOT
        )
    ...
    if input_finished_status != 0:
        return input_finished_status

    print("Step 11: Wait for IDS diagnosis to finish.")
    wait_status = wait_for_ids_scenario_complete(PROJECT_ROOT)
    if wait_status != 0:
        return wait_status

    print("Step 12: Verify the IDS prediction CSV.")
    return verify_ids_master_predictions(PROJECT_ROOT)
...
def run_scenario_command(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        print("Step 9: Tell IDS that scenario input is finished.")
        input_finished_status = mark_ids_scenario_input_finished(
            PROJECT_ROOT
        )
    ...
    print("Step 10: Wait for IDS diagnosis to finish.")
    wait_status = wait_for_ids_scenario_complete(PROJECT_ROOT)
    if wait_status != 0:
        return wait_status

    print("Step 11: Verify the IDS prediction CSV.")
    return verify_ids_master_predictions(PROJECT_ROOT)
...
def deploy_ids_inference_runtime_command(
    _: argparse.Namespace,
) -> int:
    """Deploy and validate the inference runtime on IDS EC2."""

    return deploy_ids_inference_runtime(PROJECT_ROOT)


def ids_start_inference_agents_command(
    _: argparse.Namespace,
) -> int:
    """Start both inference agents on the IDS instance."""

    return start_ids_inference_agents(PROJECT_ROOT)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    # This command uploads, installs, and validates the IDS inference runtime.
    # It reads the IDS public IP from inventory.yaml.
    # Terminal command:
    #   python3 -m aws_ids_testbed_10.cli deploy-ids-inference-runtime
    deploy_ids_inference_runtime_parser = subparsers.add_parser(
        "deploy-ids-inference-runtime",
        help=(
            "Upload, install, and validate the IDS inference "
            "runtime on AWS"
        ),
    )
    deploy_ids_inference_runtime_parser.set_defaults(
        handler=deploy_ids_inference_runtime_command
    )

    # This command starts both IDS inference agents remotely.
    # Terminal command:
    #   python3 -m aws_ids_testbed_10.cli ids-start-inference-agents
    ids_start_inference_agents_parser = subparsers.add_parser(
        "ids-start-inference-agents",
        help="Start the IDS input and inference agents on AWS",
    )
    ids_start_inference_agents_parser.set_defaults(
        handler=ids_start_inference_agents_command
    )
