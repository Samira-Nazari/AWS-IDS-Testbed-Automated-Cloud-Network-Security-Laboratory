...
def run_step_2(
    input_path: Path = STEP_1_LOADED_DATA_PATH,
    output_path: Path = STEP_2_PROCESSED_DATA_PATH,
) -> pd.DataFrame:
    """Run AWS IDS Lite V6 inference STEP=2 preprocessing."""
    # Original Lite V6 behavior:
    # input_path = BASE_DIR / "outputs" / "data" / "step_1_Loaded_Data.pkl"
    # output_path = BASE_DIR / "outputs" / "data" / "step_2_Processed_Data.pkl"
    # processed_df = preprocessor.preprocess(df, fit_scaler=True, standardize=False)
    ...
    if not input_path.exists():
        raise FileNotFoundError(
            f"STEP=1 output not found: {input_path}. "
            "Run ids-run-inference-step1 first."
        )

    logger.info("Loading IDS STEP=1 data from: %s", input_path)
    df = pd.read_pickle(input_path)
    logger.info("Loaded IDS STEP=1 data shape: %s", df.shape)
    ...
    logger.info("Saved IDS STEP=2 processed data to: %s", output_path)
    logger.info("IDS STEP=2 processed data shape: %s", processed_df.shape)
    return processed_df
...
if __name__ == "__main__":
    ...
    logger.info("")
    logger.info("=" * 80)
    logger.info("STEP 2: PREPROCESS IDS DATA (STANDALONE)")
    logger.info("=" * 80)
