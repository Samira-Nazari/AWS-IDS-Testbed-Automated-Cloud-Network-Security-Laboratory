...
def train_test_split_time_series(X, y, test_size=0.2, val_size=0.2):
    """
    Time-series safe split (NO SHUFFLE, NO LEAKAGE)
    """
    n_samples = len(X)
    ...
    train_end = n_samples - (test_split + val_split)
    val_end = n_samples - test_split
...
def train_test_split_time_series_with_gap(X, y, test_size=0.2, val_size=0.2, gap=20):
    """
    Time-series safe split with gap to avoid window overlap leakage
    """
    n_samples = len(X)
    ...
    X_val = X[train_end + gap:val_end - gap]
    y_val = y[train_end + gap:val_end - gap]

    X_test = X[val_end + gap:]
    y_test = y[val_end + gap:]

    logger.info(f"\n✓ Time-series split with gap={gap}:")
