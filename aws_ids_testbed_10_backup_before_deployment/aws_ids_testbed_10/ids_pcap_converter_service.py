    ...
    *,
    ...
    if not confirm:
        print(
            ...
            "to permanently delete IDS PCAP and CSV files."
        )
        return 2
...
IDS_BASE_DIR=/home/ubuntu/aws_ids_testbed
IDS_INPUT_DIR="$IDS_BASE_DIR/input"
IDS_CSV_DIR="$IDS_BASE_DIR/output/csv"
IDS_WORK_DIR="$IDS_BASE_DIR/tmp/pcap_to_csv"
PID_FILE="$IDS_BASE_DIR/pcap_to_csv_agent.pid"

if [ -f "$PID_FILE" ]; then
    PID="$(cat "$PID_FILE")"

    if kill -0 "$PID" 2>/dev/null; then
        echo "[ids-cleanup] Refusing deletion: converter agent is running (PID $PID)."
        exit 1
    fi
fi

PCAP_BEFORE="$(sudo find \
    "$IDS_INPUT_DIR" \
    "$IDS_CSV_DIR" \
    "$IDS_WORK_DIR" \
    -type f \( -name '*.pcap' -o -name '*.pcap.tmp' \) | wc -l)"

CSV_BEFORE="$(sudo find \
    "$IDS_INPUT_DIR" \
    "$IDS_CSV_DIR" \
    "$IDS_WORK_DIR" \
    -type f -name '*.csv' | wc -l)"

echo "[ids-cleanup] PCAP files to delete: $PCAP_BEFORE"
echo "[ids-cleanup] CSV files to delete: $CSV_BEFORE"

sudo find \
    "$IDS_INPUT_DIR" \
    "$IDS_CSV_DIR" \
    "$IDS_WORK_DIR" \
    -type f \( -name '*.pcap' -o -name '*.pcap.tmp' \) \
    -print -delete
...
PCAP_REMAINING="$(sudo find \
    "$IDS_INPUT_DIR" \
    "$IDS_CSV_DIR" \
    "$IDS_WORK_DIR" \
    -type f \( -name '*.pcap' -o -name '*.pcap.tmp' \) | wc -l)"

CSV_REMAINING="$(sudo find \
    "$IDS_INPUT_DIR" \
    "$IDS_CSV_DIR" \
    "$IDS_WORK_DIR" \
    -type f -name '*.csv' | wc -l)"

echo "[ids-cleanup] PCAP files remaining: $PCAP_REMAINING"
echo "[ids-cleanup] CSV files remaining: $CSV_REMAINING"
