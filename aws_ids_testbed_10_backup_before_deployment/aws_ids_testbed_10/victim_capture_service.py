...
def victim_delete_pcap_csv(
    ...
    *,
    ...
    if not confirm:
        print(
            "Deletion not confirmed. Run again with --confirm "
            "to permanently delete victim PCAP and CSV files."
        )
        return 2
...
VICTIM_PCAP_DIR=/opt/aws_ids_testbed/pcap
VICTIM_CSV_DIR=/opt/aws_ids_testbed/csv
...
check_agent_stopped() {
    ...
    if [ -f "$PID_FILE" ]; then
        ...
        if kill -0 "$PID" 2>/dev/null; then
            echo "[victim-cleanup] Refusing deletion: $AGENT_NAME is running (PID $PID)."
            exit 1
        fi
    fi
}
...
check_agent_stopped \
    /opt/aws_ids_testbed/continuous_capture_agent.pid \
    "capture agent"

check_agent_stopped \
    /opt/aws_ids_testbed/pending_pcap_sender_agent.pid \
    "sender agent"

PCAP_BEFORE="$(sudo find "$VICTIM_PCAP_DIR" -type f \
    \( -name '*.pcap' -o -name '*.pcap.tmp' \) | wc -l)"

CSV_BEFORE="$(sudo find "$VICTIM_CSV_DIR" -type f \
    -name '*.csv' | wc -l)"

echo "[victim-cleanup] PCAP files to delete: $PCAP_BEFORE"
echo "[victim-cleanup] CSV files to delete: $CSV_BEFORE"

sudo find "$VICTIM_PCAP_DIR" -type f \
    \( -name '*.pcap' -o -name '*.pcap.tmp' \) -print -delete

sudo find "$VICTIM_CSV_DIR" -type f \
    -name '*.csv' -print -delete

PCAP_REMAINING="$(sudo find "$VICTIM_PCAP_DIR" -type f \
    \( -name '*.pcap' -o -name '*.pcap.tmp' \) | wc -l)"

CSV_REMAINING="$(sudo find "$VICTIM_CSV_DIR" -type f \
    -name '*.csv' | wc -l)"

echo "[victim-cleanup] PCAP files remaining: $PCAP_REMAINING"
echo "[victim-cleanup] CSV files remaining: $CSV_REMAINING"

test "$PCAP_REMAINING" -eq 0
test "$CSV_REMAINING" -eq 0
"""
